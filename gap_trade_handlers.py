...
from database import get_player, save_player, player_lock_pair
...
async def cb_trade_give(cb: CallbackQuery):
    ...
    target = get_player(draft["to_uid"])
    await cb.message.edit_text(
        f"🤝 **معامله با {target['name']}**\n\n"
...
async def cb_trade_accept(cb: CallbackQuery):
    ...
    async with player_lock_pair(from_uid, to_uid):
        ...
        giver = get_player(from_uid)
        taker = get_player(to_uid)
        ...
        save_player(from_uid, giver)
        save_player(to_uid, taker)
