...
def apply_rewards(fight: SquadFight, get_player, save_player):
    ...
    for w in fight.all_warriors():
        ...
        save_player(w.uid, player)
    mvp = mvp_of(fight)
    if mvp:
        player = get_player(mvp.uid)
        if player:
            sp = player.setdefault("squad_pvp", {})
            for k in ("wins", "losses", "points", "mvp_count"):
                sp.setdefault(k, 0)
