...
def update_quest(uid: int, q_type: str, amount: int = 1):
    quests = get_today_quests()
    player = get_player(uid)
    if not player:
        return
    qp = get_qp(uid)  # مطمئن میشه امروزه و از دیتابیس تازه‌ست
    changed = False
    for q in quests:
        if q["type"] == q_type:
...
def _render_attack_panel(uid: int, player: dict) -> tuple[str, InlineKeyboardMarkup]:
...
async def resolve_hit(cb: CallbackQuery, player: dict, atk_type: str):
    ...
    if killed:
        ...
        if leveled and player.get("mentee_of"):
            _bond_mentor = get_player(player["mentee_of"])
            bond_lines = ms.check_bond_milestone(_bond_mentor, player) if _bond_mentor else []
            if bond_lines:
                save_player(player["mentee_of"], _bond_mentor)
                try:
    ...
    # Quest updates
    if killed:
        update_quest(uid, "kill", 1)
        update_quest(uid, "earn", zen_gain)
        if enemy.get("tier") == "legendary": update_quest(uid, "legend", 1)
        try:
        ...
        try:
            ...
            save_player(uid, player)  # save at line 989 already ran before this block, so save again
        except Exception:
            pass
    if atk_type == "heavy": update_quest(uid, "heavy", 1)
    if result["crit"]:      update_quest(uid, "crit", 1)
    if player["combo"] >= 10: update_quest(uid, "combo", 1)
    update_quest(uid, "survive", 1)
    ...
    if tutorial_hint:
        ...
        save_player(uid, player)  # tutorial_step/tutorial_done تغییر کرد، جداگونه ذخیره‌ش کن
